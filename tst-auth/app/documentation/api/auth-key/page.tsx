  ...
  const endpoints = [
    {
      ...
      description: 'Returns an authentication key for the account associated with. Replaces the old authentication key if it already exists.',
      queryParams: [
        { name: 'email', type: 'string', required: true, description: 'The email that is associated' },
        // { name: 'limit', type: 'number', required: false, description: 'Number of users per page.' },
      ],
      response: {
        key: "7z2+EUJ+XTZHY2t8mLJPmQ==",
        // data: [
      ...
      {endpoints.map((endpoint, index) => (
        <div key={index} className="bg-white p-6 rounded-lg shadow-md mb-6">
          ...
          {endpoint.queryParams && endpoint.queryParams.length > 0 && (
            <>
              <h3 className="mt-4 font-semibold">Query Parameters</h3>
