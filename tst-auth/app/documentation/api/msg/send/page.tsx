...
const ApiDocs = () => {
  const endpoints = [
    {
      method: 'POST',
      endpoint: '/api/msg/send',
      description: 'Sends the included message to the server that can be accessed by any other user using the route /api/msg/view.',
      queryParams: [
        { name: 'key', type: 'string', required: true, description: 'The key generated in the profile page' },
        { name: 'message', type: 'string', required: true, description: 'The message that will be sent' },
        // { name: 'limit', type: 'number', required: false, description: 'Number of users per page.' },
      ],
      response: {
        status: true,
        message: "The message was sent successfullyx"
        // data: [
